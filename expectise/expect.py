...
class Expect(object):
    ...
    # Set of classes with at least one method mocked, shared between Expect instances
    MOCKED_CLASSES = set()
    # Dict of mocked methods, with potential arguments and outputs, shared between Expect instances):
    # {
    #   (class, "method_name"):
    #     {
    #       "mock": Mock,                                   --> Mock object used to mock the target method
    ...
    def __init__(self, klass: Type) -> None:
        ...
        if klass not in Expect.MOCKED_CLASSES:
            raise EnvironmentError(
                f"""
                No method of class `{klass.__name__}` is mocked, so this instantiation is not allowed.
                Check that the right environment variable is set, and that the right methods from `{klass.__name__}`
    ...
    def context(self):
        ...
        if self._context is None:
            try:
                self._context = Expect.MOCKS[(self.klass, self.method_name)]
            except KeyError:
                raise EnvironmentError(
                    f"""
    ...
    def set_up(cls, mock: Mock) -> None:
        ...
        cls.MOCKS[(mock.klass, mock.name)] = {
            "expected": 0,
            "performed": 0,
            "decorators": [],
            "with_args": [],
    ...
    @classmethod
    def disable_mock(cls, klass: Type, method_name: str) -> None:
        """Disables a Mock."""
        if (klass, method_name) not in cls.MOCKS:
            raise EnvironmentError(
                """
        ...
        cls.MOCKS[(klass, method_name)]["mock"].disable()
    ...
    def tear_down(cls, execution_error: Optional[Exception] = None) -> None:
        ...
        message = ""
        temporary_mocks = []
        for (klass, method_name), context in cls.MOCKS.items():

            if (gap := context["expected"] - context["performed"]) > 0:
                message += f"`{method_name}` from class `{klass.__name__}` still expected to be called {gap} time(s).\n"
        ...
        for klass, method_name in temporary_mocks:
            cls.MOCKS.pop((klass, method_name))
